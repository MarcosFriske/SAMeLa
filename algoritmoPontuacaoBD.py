...
def selecionar_criterios_avaliacao(instrumento_avaliacao):
    try:
...
def carregar_lattes_xml(id_servidor):
    try:
        connection = psycopg2.connect(user=user_db,
                                      password=password_db,
                                      host=host_db,
...
def selecionar_titulos_similares(titulo, min_similaridade):
    try:
        connection = psycopg2.connect(user=user_db,
                                      password=password_db,
                                      host=host_db,
...
id_servidor = input("Digite o ID do servidor que deseja consultar: ")
xml = carregar_lattes_xml(id_servidor)
...
# Solicitação do número do instrumento de avaliação ao usuário
instrumento_avaliacao_id = input("Informe o número do ID do instrumento de avaliação: ")

# Execução da função com o ID informado pelo usuário
df_criterios_avaliacao = selecionar_criterios_avaliacao(instrumento_avaliacao_id)
